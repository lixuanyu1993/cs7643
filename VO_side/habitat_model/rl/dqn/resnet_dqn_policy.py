...
from habitat_model.rl.ddppo.policy import resnet
from habitat_model.rl.ddppo.policy.running_mean_and_var import (
    RunningMeanAndVar,
)
from habitat_model.rl.models.nn_state_encoder import NNStateEncoder
from habitat_model.rl.models.rnn_state_encoder import (
    build_rnn_state_encoder,
)
from habitat_model.rl.dqn.policy import Net, Policy


@baseline_registry.register_policy
class PointNavResNetDQNPolicy(Policy):
    def __init__(
        self,
    ...
    def from_config(
        ...
        return cls(
            ...
            network_type=config.RL.POLICY.state_encoder_network_type,
...
class PointNavResNetNet(Net):
    ...
    def __init__(
        ...
        rnn_input_size -= self._n_prev_action
        self.state_encoder = NNStateEncoder(
            input_size=(0 if self.is_blind else self._hidden_size) + rnn_input_size, 
    ...
    def forward(
        ...
        if not self.is_blind:
            if "visual_features" in observations:
                ...
                # ResNet vistual encoder 128 features with size 4x4
                visual_feats = self.visual_encoder(observations)
            
            # full connected layer, 128*4*4 -> 512
        ...
        if self.network_type == 'nn_model':
            # if it is not RNN, don't concatinate previous action
            input = torch.cat(x, dim=1)
            out = self.state_encoder( input )
        else:
            x.append(prev_actions)
            input = torch.cat(x, dim=1)

            # RNN for PPO, it cotains depth img embing 512, gps location embbings 32, previous action embings 32
            # RNN output size 512`
            out, rnn_hidden_states = self.state_encoder(
                input, rnn_hidden_states, masks
            )
