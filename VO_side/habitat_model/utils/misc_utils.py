...
def load_partial_state_dict(model, target_state_dict):
    ...
    # # ensure all keys from model_dict are in target_state_dict
    # for k in model_dict:
    #     assert (k in target_state_dict), f"{k}"
...
def image_resize_shortest_edge_mode_nearest(
        img, size: int, channels_last: bool = False
) -> torch.Tensor:
    """Resizes an img so that the shortest side is length of size while
        preserving aspect ratio.
    ...
    # w = int(w * scale)
    w, h = size
    img = torch.nn.functional.interpolate(img.float(), size=(h, w), mode="nearest").to(
        dtype=img.dtype
    )
    if channels_last:
        if len(img.shape) == 4:
...
class NearestResizer(Resizer):
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        if self._size is None:
            return input

        return image_resize_shortest_edge_mode_nearest(
